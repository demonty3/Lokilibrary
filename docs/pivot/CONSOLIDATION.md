...
**Lore-as-mechanics (year-2+).** A tier *above* lore-seeding. Lore-seeding changes how the world looks and what agents talk about; lore-as-mechanics changes how a region of the world *works*. A district built from an ingested system (e.g. *He Who Fights With Monsters*) actually runs on that system's rules — essence abilities, rank progression, affliction/resistance interplay — while a district from a different source runs on *its* system instead. The palace becomes mechanically polyglot: different wings obeying different rule-systems extracted from what the user fed in. Two users' palaces become *behaviourally* different, not just visually; agent goals become system-specific (agents in an essence district chase essences and rank up); and chaos events become system-native (an affliction outbreak instead of a generic incursion).
- **The hard problem:** extracting *runnable* mechanics from prose is the least reliable thing LLMs do — asked to invent a working system from a book, a model confabulates the gaps.
- **The fix:** the LLM never invents mechanics. You build a small library of rule *primitives* (progression ladder, resource-gated unlock, affliction/buff stack, affinity/resistance matrix, rank-gated abilities, scarcity economy). The LLM only *recognises* which primitives a source uses and *maps* the source onto them. Recognition is reliable; generation is brittle. This is the same safety pattern as agent autonomy — compose from a fixed vocabulary, never emit executable rules.
- **Graceful degradation:** sources designed as systems (HWFWM, Cradle, Mother of Learning — most progression-fantasy/LitRPG/cultivation) map cleanly and run rich mechanics; vague literary "magic" that won't map falls back to lore-as-flavour, which is still good. The product never confabulates; it maps what it can and skins the rest, and can surface the difference characterfully ("I understood this world's progression but its economy was underspecified, so I improvised").
- **Scope discipline:** ship ~6–10 primitives covering the progression-fantasy / LitRPG / cultivation cluster well — exactly the systems-loving, terminal-aesthetic, Steam-overlapping audience already targeted. Do **not** try to model every fiction's magic; a universal game-system engine is a decade-long trap and a different company.
- **Sequencing:** year-2+. Sits on top of both lore-ingestion and the agent-goal system, which must exist first. Not a v1.0 feature.
...
**Roadmap shape:** Year 1 = core single-user product. Year 2 = Workshop + community content + multi-agent society + chaos + lore-as-mechanics. Year 3 = beyond Steam (multi-source via filesystem/dream mode). Year 5+ = optionality (spatial computing port; agent-platform / MCP-native, incl. MCP-as-outward-facing-API).
