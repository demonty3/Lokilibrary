...
**Valve AI disclosure status (Jan 2026 policy).** LibraryWorld is *both* **Pre-Generated** (Stages 2–6 — baked assets that ship with the build) *and* **Live-Generated** (Stage 1 — Claude runs against the user's profile at runtime). Steam Direct's Content Survey requires disclosure of both categories, and Live-Generated triggers an additional "guardrails disclosure" — what prevents the runtime call from producing illegal or harmful content. For Stage 1 the guardrails are: structured-JSON-only output, whitelisted scene templates / archetypes / asset IDs (server-side validation rejects anything off-list before the frontend sees it), no free-form image / audio / 3D generation at runtime, and Anthropic's own content-safety layer on Claude. The store-page disclosure is a v1.0 launch obligation — see §11 and PLAN.md "Beyond Phase 7."
...
- **v1.0 — Steam launch.** Desktop app shipped on Steam as a one-time ~$7–10 purchase. 3–5 hand-built templates, share artifacts, "Year in Library" annual moment. No Workshop yet. The launch itself has its own checklist of Steam Direct obligations (paperwork, 30-day wait, AI Content Survey, Coming Soon page) tracked in PLAN.md "Beyond Phase 7."
- **v1.x** — Steam Workshop integration (community-built templates — the long-term moat), template authoring tool, friend-comparison feature. **Workshop content is free, never monetized** — Wallpaper Engine tried a paid item store and abandoned it for verification, codec-licensing, and buyer-confusion reasons; we don't repeat that mistake. The moderation pipeline (§11.workshop-moderation) is a v1.x prerequisite, not a follow-up.
...
- **Valve AI policy — Live-Generated guardrails (v1.0 launch obligation).** Per §6.1, the Stage 1 runtime call puts LibraryWorld in Valve's "Live-Generated" disclosure category as well as "Pre-Generated." The Content Survey at submission time requires describing both, and Live-Generated additionally requires a guardrails statement. The policy was rewritten Jan 2026 and is "voluntary and not enforced by moderation" per current GameSpot reporting, but the policy direction is tightening; build the pipeline so we can ship without any runtime AI in an emergency (curated default-template pool as fallback).
- **Workshop moderation timebomb (v1.x prerequisite).** Once Workshop opens, NSFW imagery, IP-infringing fan-art reuse, and embedded prompts-that-generate-bad-content-on-someone-else's-machine all arrive on day one — Wallpaper Engine has dealt with all three for years. The moderation pipeline must exist *before* Workshop opens, not after: (1) Workshop templates ship as **static baked assets only** — no live AI generation from community templates allowed (our Stage 1 pipeline is first-party only); (2) pre-publish moderation queue with an image-moderation API check on every preview (Cloudflare Images, Hive, or AWS Rekognition); (3) DMCA flow on our own site supplementing Valve's light-touch system; (4) remote kill-switch in Cloudflare Workers — the desktop app checks on launch and refuses to load any blocked template id.
- **Steam Direct launch logistics.** v1.0 ship is gated by paperwork, not code. Specifically: **$100 USD non-refundable Direct fee** (recoupable as credit against first $1,000 GAR); **30-day mandatory wait** between fee payment and release; **Coming Soon page ≥2 weeks public before launch** (wishlist accumulation — the algorithm signal); **AI Content Survey** for both Pre- and Live-Generated stages; Steamworks partner onboarding (start ahead of Phase 6 — review queue is the long pole). Plan a 6–8 week pre-launch window from "feature freeze" to "release-day," because the Steamworks queue and the Coming Soon clock both run in parallel.
