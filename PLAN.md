...
| Vertical stacking — underground continuation | **PROBE OPEN 2026-08-17 (mockup-first; eyeball pending).** A window snapped UNDERNEATH = the same wing's deep strata (no sky), downward only, under window 640×260. Bars frozen + committed before authoring, inheriting the Terrace Join kill verbatim: `docs/design-reviews/2026-08-17-underground-continuation.{md,html}`. Engine slices (VJoin topology → under-compose + seam craft → shaft-column descent) are GATED on the eyeball; outline in the approved plan. STATE.md has the record |
