...
| Claude-authoring rung 1 — the mural blueprint + cold test | **RUNG CLOSED 2026-08-24 — ALL FOUR Done-means bars MET. Matching (4b) PASSED 4/4: Harry's real library pulled from his steamid (5 games), three engineered fakes + the real profile each got a context-cold mural (zero hand-fixes), and Harry paired all four blind — 1-A finisher, 2-D REAL, 3-C collector, 4-B cozy — against the pre-frozen ≥2-of-4 cut-off; no kill fired anywhere on the rung. The MCP builder's-hand rung's spec-interview is UNBLOCKED. Artefacts: docs/design-reviews/2026-08-23-mural-matching.{html,key.md}. Earlier bars: corpus eyeball PASSED, bars FROZEN, game-IP rule relaxed (dated CLAUDE.md amendment), cold test MET (3 murals, zero hand-fixes, hard rails held), blind A/B PASSED and the cold murals MERGED unedited (806cb20; desk carries six). Haiku-floor run stays optional/informative.** Spec frozen before implementation (`docs/superpowers/specs/2026-08-21-claude-authoring-rung1-mural-blueprint.md`, 055d568; Harry's four calls: rung 1 only, queued behind dungeon rung 4, both briefs with Claude's per-wing choice, blind A/B + matching). Registry `src/murals/` (22×5 glyph grids + palette-KEY ink, bg/bgAlt/fgBright structurally unspellable), seed corpus of 4 (d0 stardew world / d2 disco relationship / d3 hades world / d5 hollow relationship, one-line whys on the record), gate smoke-mural-blueprint 138 (per-wing + --values; bars PROVISIONAL until the corpus eyeball), desk mounts authored wings only (rect returns where art hangs; CDN path stays dormant), blueprint `docs/blueprints/mural.md` with the d0 worked example verified end-to-end + the exemplar-content guard. Zero runtime AI calls. 85-smoke sweep + typecheck green; shots `docs/design-reviews/2026-08-23-mural-seed-corpus/`. Kill (armed): cold output needs hand-fixes / A/B fails → the rung dies and the MCP + first-run rungs never start; matching at chance → personalisation not real. STATE.md has the record |
