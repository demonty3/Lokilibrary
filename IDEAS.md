...
| Far-layer parallax | **NEEDS-CHECK CLOSED 2026-08-21 — no mechanism.** Re-arms if any slice ever gives a window a camera. | The check's premise ("each window scrolls its camera independently") is false of the shipped desk: **the desk has no horizontal camera at all.** `world.x` is a one-time centring offset set by `layoutWorld` at mount (`src/terminal/terminalLand.ts:965`; the code's own words at `:873` — "`world` has no camera, scale fixed at mount, x/y moved only by layoutWorld"), and width means *horizon* (variable-widths, 2026-08-18), so a wider window shows more world rather than scrolling it. With no scroll, the far plane cannot desynchronise across a seam — the failure mode has no mechanism, and there is nothing to screenshot. *(was)* Check = screenshot two joined terminals scrolled apart. **Re-arm condition, named:** any future slice that pans a window's view (a camera, a drag-to-scroll, an autonomous wallpaper camera) must anchor the far layer to shared desk-space and re-runs the original check; Addendum-9's undercroft scroll is vertical and peek-only, scrolling the under-pane, not the horizon — it does not trigger this on its own, but the autonomous-camera candidate named there does. |
