...
| **Daylight colour, the other seven packs** | **OPEN, unscheduled** | Three packs are authored (`phosphor` boot/lift+hue, `catppuccin-mocha` lift, `solarized-dark` hue); the remaining seven omit `daySky` and are byte-identical to the pre-register desk, which is legal and deliberate. Authoring one is ~two hex values plus `npx tsx scripts/smoke-style-pack.mts <id> --values` to find its ceiling. **Do it hue-first** — the judged finding is that hue tells time better than brightness. `gameboy-dmg` should probably stay omitted: it deleted its sky roles outright and omission is its identity. |
